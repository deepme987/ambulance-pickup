...
def plot(_persons, _hospitals, fname="result.txt"):
    ...
    hospitals = {key: val for hospital in hospitals for key, val in hospital.prettify().items()}
    ...
    for i, color in enumerate(colors):
        temp = persons[(min_ + (i * bins) <= persons["time"]) & (persons["time"] <= min_ + ((i + 1) * bins)) & (
    ...
    for i, (hosp, val) in enumerate(vals.items()):
        ...
        ax.scatter(x=hospitals.iloc[hosp - 1]["x"], y=hospitals.iloc[hosp - 1]["y"],
                   marker='s', s=100, label='Hospital' + str(hosp), color=colors[i])
